    ...
    return new Promise((resolve, reject) => {
        ...
        function addDirectory(dirPath, archivePath = '') {
            ...
            for (const entry of entries) {
                ...
                const relativePath = archivePath ? path.join(archivePath, entry.name) : entry.name
...
async function sendBackupToOwner(backupInfo) {
    ...
    try {
        ...
        console.log(`[AutoBackup] Backup sent successfully to owner (${sizeInMB} MB)`)
        return true
...
function startAutoBackup(sock) {
    ...
    console.log(`[AutoBackup] Started with interval: ${formatInterval(state.intervalMs)}`)
