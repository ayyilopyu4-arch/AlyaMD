...
async function handler(m, { sock }) {
    ...
    if (!action) {
        ...
        txt += `┃ 📅 Last Backup: ${status.lastBackup ? new Date(status.lastBackup).toLocaleString('id-ID') : '-'}\n`
    ...
    switch (action) {
        ...
        case 'info': {
            ...
            txt += `┃ 📅 Last: ${status.lastBackup ? new Date(status.lastBackup).toLocaleString('id-ID') : '-'}\n`
            txt += `┃ #️⃣ Total: ${status.backupCount} backup\n`
            txt += `┃ 📤 Target: ${ownerNum}\n`
            txt += `╰┈┈┈┈┈┈┈┈⬡`
