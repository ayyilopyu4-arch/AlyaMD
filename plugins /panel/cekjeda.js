...
const { hasAccessToServer, VALID_SERVERS } = require('../../src/lib/cpanelRoles')
...
async function handler(m, { sock }) {
    const hasAccess = VALID_SERVERS.some(server => hasAccessToServer(m.sender, server, m.isOwner))
    ...
    if (lastUsed > 0) {
        ...
        text += `┃ ◦ Last create: *${lastUsedDate.toLocaleTimeString('id-ID')}*\n`
    }
